...
propScoreM2 = function(X, W, h){ # Or a function of Z
  ...
  eta = -0.5 + exp(-((t-0.5)/0.3)^2-(X/5)^2)
  -W[1] + 0.5*W[2] - 0.1*W[3] + trapz(t, eta)
}
...
data_simu_M2 = t(sapply(1:100000, function(k){
  ...
  T  = rbinom(n = 1, size = 1, prob = boot::inv.logit(propScoreM2(X,W, h = 0.02)))
  Y  = treatEff(T, Z)
...
Model2_T2 = foreach(n = c(200,500), .combine = 'acomb3') %:%
  foreach(B = 1:1000, .combine = 'rbind') %dopar% {
    set.seed(2000+B)
    data_raw = data_simu_M2[sample.int(nrow(data_simu_M2), n, replace = F), ]  # sample 200 rows from data_simu
...
save(Model2_T2, file = "Model2_T2.rda")
